...
// Mobile push relaying is opt-in. Unless PUSH_RELAY_ENABLED is explicitly set to
// 'true', no installation is registered with Bitwarden and no device token or
// notification metadata ever leaves the deployment.
export function isPushRelayEnabled(env: Env): boolean {
  return String(env.PUSH_RELAY_ENABLED || '').trim().toLowerCase() === 'true';
}
...
export function mobilePayloadFromSignalR(updateType: number, userId: string, revisionDate: string, payload: Record<string, unknown> | null | undefined): Record<string, unknown> {
...
export async function registerMobilePushDevice(
  ...
  if (!isPushRelayEnabled(env)) return false;
  const credentials = await ensurePushInstallationCredentials(env.DB);
...
export async function notifyMobilePush(
  ...
  if (!isPushRelayEnabled(env)) return;
