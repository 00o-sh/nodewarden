...
export class StorageService {
  ...
  async initializeDatabase(): Promise<void> {
    ...
    // Push-installation credentials are provisioned lazily by the push-relay
    // entry points (device registration / notification send), which only run
    // when a real mobile push device exists. Eagerly provisioning here forced
    // an outbound Bitwarden API call on every cold start (silently registering
    // an installation), so it is intentionally omitted.
